class AEScode:
    ...
    def __init__(self, key):
        """初始化AES加密器，生成轮密钥"""
        if len(key) != 16:
            raise ValueError("AES-128要求16字节(128位)的密钥")
        self.key = key
    ...
    def encrypt(self, plaintext):
        ...
        for i in range(0, len(padded), 16):
            block = padded[i:i + 16]
            cipher_block = self._encrypt_block(block)
    ...
    def _mix_columns(self, state):
        ...
        for i in range(4):
            col = state[i * 4:(i + 1) * 4]
            state[i * 4:(i + 1) * 4] = self._mix_single_column(col)
        return state
    ...
    def _inv_mix_columns(self, state):
        ...
        for i in range(4):
            col = state[i * 4:(i + 1) * 4]
            state[i * 4:(i + 1) * 4] = self._inv_mix_single_column(col)
        return state

    def _add_round_key(self, state, key):
        """轮密钥加操作"""
        return [si ^ ki for si, ki in zip(state, key)]

    def _key_expansion(self, key):
        """密钥扩展，生成11轮密钥（1轮初始密钥+10轮轮密钥）"""
        Nk = 4  # 密钥字数量
        Nr = 10  # 轮数
...
if __name__ == "__main__":
    ...
    # print("加密结果:", cipher.hex())  # 应输出: 3925841d02dc09fbdc118597196a0b32

    # 解密测试
    # decrypted = aes.decrypt(cipher)
    # print("解密结果:", decrypted.hex())  # 应输出原始明文的十六进制
